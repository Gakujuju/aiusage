...
## D27. 内訳の欠落は cost_source ではなく専用のフラグで表す

Codex のデスクトップ版セッションは合計トークンだけを報告し、
入力と出力の内訳を持たない（`total_token_usage` も全0）。
比率は復元できないので、合計を `input_tokens` に入れて数だけ合わせ、
`breakdown_missing` を立てる。

**cost_source に新しい値を足さなかった。**
あれは「価格が分かるか」の軸で、これは「形が分かるか」の軸。
1つの値に2軸を載せると片方が見えなくなる。
実際この行は cost_source='unknown'（価格不明）かつ
breakdown_missing=1（内訳不明）で、両方が同時に真になる。

### なぜ専用の列ではなく input に入れるのか

案として「合計専用の列を足す」も検討したが、採らなかった。
失敗の仕方が違う。

  専用列の失敗   : 集計側が新しい列を足し忘れる → **数字が消える**
  input+旗の失敗 : 画面が旗を無視する → 数字は残り、ラベルが不正確

このプロジェクトが繰り返しているのは前者である
（cost_source='pricing' の誤設定、merge の platform 欠落、
Codex の13%欠落）。同じ病気を新しい列で再発させない。

代償は明示しておく。**この行の「入力」は入力+出力の合算**であり、
入出力比を見る用途では正しくない。/overview に注記を出している。

## D28. mergeSyncedRecordsIntoRecords は挿入専用。更新は伝播しない

`WHERE r.id IS NULL` + `INSERT OR IGNORE` なので、
**すでに records にある行は二度と更新されない。**
spoke 側でどれだけ正しく直しても、ハブの表示は変わらない。

これは意図ではなく欠陥である。該当するのは削除だけではない:
platform の埋め戻し、recalc、今回のコスト是正、すべてが届かない。
D25（削除は伝播しない）は仕様として書いたが、
**更新が伝播しないのは仕様ではない。**

実測: ノートPC由来の codex 72行は synced_records と records の
両方にあり、synced_records を更新しても records は変わらない。

`updated_at` を見て更新する形に直すのが筋だが、
**旧コードの spoke が残っている間に入れてはいけない。**
誤った値がハブの正しい値を上書きで巻き戻す。
全端末の更新が済んでから着手する。それまでは、
ハブ側の是正は spoke の再送に頼らず直接行う。

## D29. ウィジェットはローカルDBを読まず、ハブへログインして読む

**決定**: ウィジェットは better-sqlite3 を持たない。
ハブへ HTTP でログインし、`/api/quotas` と `/api/notifications` を読む。
**ハブ自身も同じ経路を通る**（自分の 127.0.0.1 に対して）。
端末によって読み方を変えない。

### 理由

**1. better-sqlite3 が消え、ABI 不一致という問題の種類ごと無くなった。**
Node の ABI と Electron の ABI は違う。同じ `.node` を両方から使えない。
`@electron/rebuild` で作り直すと、今度は **CLI 側の better-sqlite3 が壊れる**
（2026-09-02、職場PC。OPERATIONS.md に復旧手順あり）。
一方を直すと他方が壊れる形だったので、片方を消した。

**2. spoke が Claude の資格情報を持たなくてよくなった。**
職場PCで Claude の利用枠が見えなかった元の問題が、これで解決した。
枠を取りに行くのはハブだけでよい。

**3. 通知が「その端末のもの」から「全端末のもの」になった。**
ローカルDBを読む形では、その端末で起きたことしか出せなかった。

### 代償（3つとも実在する）

**・serve が止まるとウィジェットは何も出せない。**
以前はローカルDBがあったので、ハブが落ちても自分の分は出せた。
いまは何も出せない。**古い数字を黙って出すよりは良い**という判断だが、
失うものが無いわけではない。

**・読み手が増えると上流への呼び出しが増える。**
`/api/quotas` は毎リクエスト `queryAllQuotas()` を実行する。
ウィジェット1台につき毎分2回（トレイとパネルが別々に叩く）。
台数分に増える。**2026-09-02 に 429 が出た**（OPERATIONS.md に構造の記録）。
対処は `/api/quotas` を `quota_current` から返す形にすること。

**・職場PCに、ハブの内容を読めるパスワードが平文で1つ増えた。**
`config.credentials.hubDashboardPassword`（0600）。
ハブは `includeAssistantMessage: true` である。
**通知本文には AI の応答の抜粋が入っている。3台分すべての。**
つまり、そのパスワードで読めるのは数字ではなく**作業の内容**である。
方向も逆になった ── 従来は 職場PC→ハブ の一方向だったものが、
職場PC からハブの中身を読む向きが増えた。
書き込み権限が増えたわけではない。そこは変わっていない。

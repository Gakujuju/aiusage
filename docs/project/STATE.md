...
## 2026-09-04 畳んだら設定を閉じる ＋ ダブルクリックで畳む

    畳む      setCollapsed(true) の中で showSettings = false。▾ もダブルクリックも同じ関数を通る
    開く      帯は1クリック（遅延なし）。帯にダブルクリックは付けない
    ガード    帯から開いた直後 400ms は開いた窓のダブルクリックを無視（指示の数字。未測定）
    除外      button / input / select / textarea / a の上のダブルクリックはそのコントロールのもの
    ドラッグ  開いた窓のドラッグはヘッダの app-region（ネイティブ）なので、そこにダブルクリックは届かない。
              帯側はしきい値 4px のまま

実測: 設定→▾→帯→開く = 本体 ／ 設定→本体の題名をダブルクリック→帯→開く = 本体 ／
帯をダブルクリック = 開いたまま ／ ヘッダを掴んで動かしてから2連打 = 開いたまま。

検証で踏んだ: 設定パネルは高さが作業領域ちょうど（1032）で、再レイアウトのたびに
アプリが窓を上端（T=0）へ再アンカーする。座標を「置いた位置」で固定した検証は外れる。
**クリックの直前に毎回 GetWindowRect で読む。** 外れた1回は言語ボタンに当たって中文に変わっていた
（除外規則が効いた証拠でもある）。スクラッチの補助スクリプトは**ターンごとに消える**ので、使う直前に書く。

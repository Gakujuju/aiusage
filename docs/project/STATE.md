...
| 8-A-3 | 通知の設定と送信履歴 | 完了 |
| 8-B-1 | Tailscale 経由での到達性 | 実装完了・本番未適用 |
| 8 | Android/PWA 化 | 未着手（8-B-2 / 8-B-3 が残り） |

## 未消化の作業

- Phase 5 のクローズ確認のうち、スクラッチ作業での通知1点
  （実プロジェクトでの動作は 2026-08-30 13:27 のセッションで確認済み）
- Phase 8-B-1 の本番適用。ダッシュボードパスワードが未設定のため保留。
  ユーザーが aiusage set-dashboard-password を実行したら適用する。
- serve の起動が永続化されていない（再起動で止まる）。8-B-1 の報告に提案あり。

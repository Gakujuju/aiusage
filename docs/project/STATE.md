...
── どれも「そこを通ったことがなかった」。
ここは**そうと分かっている未走行区間**なので、名前を付けて置いておく。

### 未走行の分岐は2つある

    shouldHideWindowOnBlur(isPackaged) → isPackaged
    ...
    shouldShowWindowOnLaunch(isPackaged) → !isPackaged
      3台では常に true。つまり**未走行なのは false のほう**である。
      packaged にすると**起動時に窓が出なくなる。**
      ★ こちらのほうが重い。トレイのアイコンだけが手がかりになるが、
      Windows 11 は既定でトレイアイコンを隠す（2026-09-02 に実測）。
      **packaged の初回起動は、画面上に何も出ない可能性がある。**
      401 と config 破損の経路は窓を強制的に出すようにしてあるので、
      そこは通る。通らないのは**正常起動**である。
...
直す方向は2つ。**まだ決めていない:**
    ...
    (a) 眠っていた時間を差し引く（稼働時間で測る）
    (b) 眠りから戻った直後は判定から除く（猶予を置く）

いま直さないのは、**どちらが正しいか実測していない**ため。
ノートPCが1日に何回・どれだけ眠るかを見てから決める。
**ただし放置ではない。** 放置すると、
1 を黙らせた意味がこの1件で相殺される。
